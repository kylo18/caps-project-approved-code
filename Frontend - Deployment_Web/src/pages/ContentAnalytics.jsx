...
const ContentAnalytics = () => {
  ...
  // Fallback sample — all zeros when no real data
  const viewed    = data?.mostViewed     || [];
  const attempted = data?.mostAttempted  || [];
  const errors    = data?.highestError   || [];
  const skipped   = data?.mostSkipped    || [];
  ...
  const maxVal  = current?.items?.length ? Math.max(...current.items.map(i => i.count ?? i.views ?? i.rate ?? 0)) : 1;
          ...
          <div style={{ padding: "16px 20px" }}>
            {loading ? (
              ...
              current.items.map((item, i) => {
                const val    = item.count ?? item.views ?? item.rate ?? 0;
                ...
                return (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0", borderBottom: "1px solid #F8F6F3" }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 13, fontWeight: 500, color: "#1A1814", marginBottom: 4 }}>{item.name || item.lessonName || item.questionText || "—"}</div>
