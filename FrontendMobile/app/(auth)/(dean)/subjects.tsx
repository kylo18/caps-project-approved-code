// ─────────────────────────────────────────────────────────────────────────────
// Purpose: Admin "Subject Questions" screen — fetches all subjects and their
//          questions. Provides full CRUD-lite: add questions, toggle approval
//          status, delete questions, and navigate to edit. Admins can review
//          and moderate questions across the entire platform.
// Key sections: Header with back button, horizontal subject tabs, questions list
//               (with status toggle, delete, and edit actions), empty state,
...
export default function AdminSubjectsScreen() {
  ...
  const fetchSubjects = async () => {
    ...
    try {
      ...
      // Original: Array.isArray(data.subjects) ? data.subjects : []
  ...
  const fetchQuestions = async () => {
    ...
    try {
      ...
      // Original: Array.isArray(data.questions) ? data.questions : []
  ...
  const handleToggleStatus = async (questionID, currentStatus) => {
    try {
      ...
      await apiRequest(`/api/questions/${questionID}/status`, {
        method: 'PATCH',
          ...
          {selectedSubject && (
            <View style={styles.questionsSection}>
              <View style={styles.questionsHeader}>
                ...
                <TouchableOpacity style={styles.addBtn} onPress={() => router.push({ pathname: '/(auth)/practice-exam/add-question', params: { subjectID: selectedSubject.subjectID } })} activeOpacity={0.7}>
              ...
              {questions.length === 0 ? (
                <View style={[styles.emptyState, { backgroundColor: colors.card }]}>
                  ...
                  <Text style={[styles.emptyText, { color: colors.textSecondary }]}>Add questions for this subject.</Text>
                </View>
              ) : (
                questions.map((q, idx) => (
                  <View key={q.questionID || idx} style={[styles.questionCard, { backgroundColor: colors.card }]}>
                    ...
                    <View style={styles.questionFooter}>
                      ...
                      <TouchableOpacity onPress={() => router.push({ pathname: '/(auth)/practice-exam/edit-question', params: { questionID: q.questionID } })}>
