...
import { getFacultyClasses } from '../../../src/services/facultyClassService';

export default function AssociateDeanClassesScreen() {
  ...
  const loadClasses = useCallback(async () => {
    try {
      ...
      console.error('Error loading associate dean classes:', error);
  ...
  useScreenFloatingTools([
    {
      ...
      icon: 'add-outline',
      label: 'Create Class',
      onPress: () => router.push('/(auth)/(associate-dean)/class-detail'),
    },
    {
      key: 'archived',
      icon: 'archive-outline',
      label: 'Archived',
      onPress: () => router.push('/(auth)/(associate-dean)/archived-classes'),
    },
  ]);
  ...
  if (loading) {
    return (
      <View className={`flex-1 ${isDark ? 'bg-black' : 'bg-gray-100'}`} style={{ paddingBottom: insets.bottom + 12 }}>
        <MobileHeader title="College Classes" />
          ...
          <View className="flex-row items-center justify-between">
            <View className="flex-1 mr-3">
              ...
              <Text className={`mt-1 text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                Review college classes, manage students and assigned quizzes, and keep class settings aligned with the web flow.
              </Text>
            </View>
            <TouchableOpacity
          ...
          filteredClasses.map((item, index) => {
            ...
            return (
              <TouchableOpacity
                ...
                onPress={() =>
                  router.push({
                    pathname: '/(auth)/(associate-dean)/class-detail',
                    params: {
