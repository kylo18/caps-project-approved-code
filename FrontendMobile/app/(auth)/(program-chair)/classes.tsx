  ...
  useScreenFloatingTools([
    {
      ...
      icon: 'add-outline',
      label: 'Create Class',
      onPress: () => router.push('/(auth)/(program-chair)/class-detail'),
    },
    {
      key: 'archived',
      icon: 'archive-outline',
      label: 'Archived',
      onPress: () => router.push('/(auth)/(program-chair)/archived-classes'),
    },
  ]);
  ...
  if (loading) {
    return (
      <View className={`flex-1 ${isDark ? 'bg-black' : 'bg-gray-100'}`} style={{ paddingBottom: insets.bottom + 12 }}>
        <MobileHeader title="Program Classes" />
          ...
          <View className="flex-row items-center justify-between">
            <View className="flex-1 mr-3">
              ...
              <Text className={`mt-1 text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                Review classes, manage enrolled students, assign quizzes, and keep class settings aligned with the web flow.
              </Text>
            </View>
            <TouchableOpacity
        ...
        {filteredClasses.length === 0 ? (
          <View className={`rounded-2xl p-8 items-center ${isDark ? 'bg-gray-900' : 'bg-white'}`}>
            ...
            <Text className={`mt-2 text-center text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
              {classes.length === 0
                ? 'Program classes will appear here once they are available.'
          ...
          filteredClasses.map((item, index) => {
            ...
            return (
              <TouchableOpacity
                ...
                onPress={() =>
                  router.push({
                    pathname: '/(auth)/(program-chair)/class-detail',
                    params: {
