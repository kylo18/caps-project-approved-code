...
import { unregisterStoredPushToken } from '../../../src/services/pushNotificationService';
...
function TrendBadge({ trend }: { trend?: string }) {
  if (!trend || trend === 'stable') {
    return (
      <View className="flex-row items-center rounded-full px-2 py-1" style={{ backgroundColor: 'rgba(255,255,255,0.2)', gap: 4 }}>
        <Ionicons name="remove" size={12} color="#fff" />
        <Text style={{ color: '#fff', fontFamily: 'Rubik', fontSize: 11, fontWeight: '600' }}>Stable</Text>
      </View>
    );
  }
  ...
  return (
    <View className="flex-row items-center rounded-full px-2 py-1" style={{ backgroundColor: 'rgba(255,255,255,0.2)', gap: 4 }}>
      <Ionicons name={isImproving ? 'trending-up' : 'trending-down'} size={12} color="#fff" />
      <Text style={{ color: '#fff', fontFamily: 'Rubik', fontSize: 11, fontWeight: '600', textTransform: 'capitalize' }}>
        {trend}
      </Text>
    </View>
  );
...
export default function StudentInsightsScreen() {
  ...
  async function loadInsights(options?: { silent?: boolean }) {
    ...
    try {
      ...
      // Fetch recommendations using latest attempt
      const latestAttempt = trendResponse?.data?.[0];
      if (latestAttempt?.result_id) {
        try {
          setRecommendationsError(false);
          const rec = await getRecommendations(latestAttempt.result_id);
          if (rec.error) {
      ...
      >
        ...
        >
          ...
          <View className="mb-[18px]">
            ...
            >
              Your account details and learning overview
            </Text>
          </View>

          <View
            className="rounded-[28px] p-4"
            style={{ backgroundColor: 'rgba(255,255,255,0.16)', ...studentShadow }}
          >
                ...
                <View className="mt-3">
                  <View className="flex-row items-center gap-2">
                    ...
                    <TouchableOpacity
                      ...
                      className="p-1"
                    >
                      <Ionicons name="copy-outline" size={12} color="rgba(255,255,255,0.78)" />
                    </TouchableOpacity>
                  </View>
                  <Text
                    className="mt-1"
              ...
              >
                <View>
                  <StudentAvatar
                    ...
                    size={68}
                    index={2}
                    style={{ borderWidth: 3, borderColor: 'rgba(255,255,255,0.72)' }}
                  />
                  <View
                    className="absolute -right-1 -bottom-1 w-7 h-7 rounded-full items-center justify-center"
                    style={{
                  ...
                  >
                    <Ionicons name="create-outline" size={14} color={studentColors.orange} />
                  </View>
                </View>
                <Text
                  className="mt-2"
        ...
        <View className="bg-white rounded-t-[34px] -mt-7 px-6 pt-6 pb-7 min-h-[620px]">
          ...
          {/* ── Student Insight ──────────────────────────────────────────── */}
          <StudentSectionHeader title="Student Insight" style={{ marginTop: 22 }} />
