  ...
  useScreenFloatingTools([
    {
      ...
      icon: 'add-outline',
      label: 'Create Class',
      onPress: () => router.push('/(auth)/(faculty)/class-detail'),
    },
    {
      key: 'archived',
      icon: 'archive-outline',
      label: 'Archived',
      onPress: () => router.push('/(auth)/(faculty)/archived-classes'),
    },
  ]);
        ...
        <View className={`rounded-2xl p-5 ${isDark ? 'bg-gray-900' : 'bg-white'}`}>
          <View className="flex-row items-center justify-between">
            <View className="flex-1 mr-3">
              <Text className={`text-xl font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>Class Management</Text>
              <Text className={`mt-1 text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                Open a class to manage students, assigned quizzes, dates, and class settings.
              </Text>
            </View>
            <TouchableOpacity
        ...
        {filteredClasses.length === 0 ? (
          <View className={`rounded-2xl p-8 items-center ${isDark ? 'bg-gray-900' : 'bg-white'}`}>
            ...
            <Text className={`mt-2 text-center text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
              {classes.length === 0
                ? 'Your assigned classes will appear here once they are available.'
          ...
          filteredClasses.map((item, index) => {
            ...
            return (
              <TouchableOpacity
                ...
                onPress={() =>
                  router.push({
                    pathname: '/(auth)/(faculty)/class-detail',
                    params: {
