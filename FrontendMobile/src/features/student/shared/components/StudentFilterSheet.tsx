  ...
  return (
    <Modal transparent visible={visible} animationType="slide" onRequestClose={onClose}>
      <Pressable className="flex-1 justify-end" style={{ backgroundColor: 'rgba(12,9,42,0.35)' }} onPress={onClose}>
        <Pressable className="bg-white rounded-t-[32px] px-5 pt-3 pb-6" style={{ maxHeight: '80%' }} onPress={(e) => e.stopPropagation()}>
          ...
          >
            ...
            >
              <Text
                className={`font-sans text-[13px] leading-5 ${selectedProgramID === null ? 'text-white font-bold' : 'text-[#858494] font-medium'}`}
              >
                All Programs
              </Text>
            </Pressable>
          ...
          >
            ...
            {subjects.map((subject) => (
              ...
              >
                ...
                {subject.subjectCode ? (
                  <Text className="font-sans text-[#858494] text-xs font-normal leading-[18px] mt-0.5">{subject.subjectCode}</Text>
                ) : null}
              </Pressable>
            ))}
          </ScrollView>
