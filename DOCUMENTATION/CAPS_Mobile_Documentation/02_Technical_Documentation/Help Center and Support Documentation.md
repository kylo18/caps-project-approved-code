...
## Purpose
This document explains the currently implemented Help Center and Support features in the CAPS mobile application based on the `FrontendMobile` folder. It covers the student-facing FAQ and support request flow, the role-based behavior of the shared help modal, and the administrator support ticket management workflow backed by the mobile app and backend API.
...
- posting system announcements from the same shared modal for non-student users
...
## Notes
- This document was written to match the currently implemented `FrontendMobile` support workflows.
