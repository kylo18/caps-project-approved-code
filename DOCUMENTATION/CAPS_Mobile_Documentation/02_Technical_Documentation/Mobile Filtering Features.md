...
## Purpose
This document explains the filtering features that are currently implemented in the CAPS mobile application based on the `FrontendMobile` folder. It focuses on actual user-facing filter behavior found in the current mobile frontend and avoids unsupported assumptions.
...
Frontend areas referenced by this guide:
- student routes under `app/(auth)/(student)`
- role user-management screens under `app/(auth)/(faculty)`, `app/(auth)/(program-chair)`, `app/(auth)/(dean)`, and `app/(auth)/(associate-dean)`
- `src/features/classes/screens/ClassesScreen.tsx`
- `src/services/studentLeaderboardService.ts`
- `src/features/student/shared/components/StudentFilterSheet.tsx`
...
- Keep this guide scoped to `FrontendMobile`; web-only filtering behavior should be documented separately.
