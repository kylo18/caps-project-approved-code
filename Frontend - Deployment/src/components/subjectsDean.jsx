...
const SideBarDropDown = ({
  ...
  className,
  selectedSubject,
  showDirectly = false, // New prop to show list immediately
}) => {
  const [subjects, setSubjects] = useState([]);
  const [filteredSubjects, setFilteredSubjects] = useState([]);
  const [searchTerm, setSearchTerm] = useState("");
  // Initialize isOpen to true if we are showing it directly
  ...
  return (
    <div className="">
      {/* 
        Hide the sidebar icon if showDirectly is true. 
        This is used for the mobile modal view. 
      */}
      {!showDirectly && (
        <li
        ...
        <>
          <div
            className={`subject-selector-theme open-sans sm:bg-opacity-0 flex flex-col border-gray-300 dark:border-white/10 bg-white transition-all duration-200 ease-in-out dark:bg-black ${
              showDirectly 
                ? "relative w-full border-none h-auto" // Mobile Modal styling: no fixed, no h-screen
                : "fixed inset-0 z-50 h-[100vh] border-r sm:hidden lg:inset-auto lg:top-0 lg:left-[55px] lg:flex lg:w-62 lg:translate-x-0 lg:translate-y-0 lg:border-l lg:shadow-none" // Original sidebar styling
            } ${
              isOpen ? "translate-x-0" : "translate-x-full sm:translate-x-0"
            } ${window.innerWidth < 640 && !showDirectly ? (isOpen ? "animate-fade-in-up" : "") : !showDirectly ? "animate-fade-in-left" : ""}`}
          >
            {/* 
              Hide the 'Select a Subject' header inside the component 
              if showDirectly is true, because Layout.jsx already has a header for the modal.
            */}
            {!showDirectly && (
              ...
              <div className="w-full" ref={dropdownRef}>
                ...
                {showYearSubjects && (
                  <ul className="animate-dropdown animate-fadein absolute left-0 z-10 mx-2 mt-1 w-[calc(100%-16px)] rounded border border-gray-300 bg-white dark:border-white/10 dark:bg-[#1a1a1a] p-1 shadow-lg">
                    <li
                      className="flex cursor-pointer items-center gap-2 px-1 py-2 text-sm hover:bg-orange-50 dark:hover:bg-white/5 dark:text-gray-200"
                      onClick={() => {
                    ...
                    {yearLevelOptions.map((yearLevel) => (
                      <li
                        key={yearLevel}
                        className="flex cursor-pointer items-center gap-2 rounded-sm px-1 py-2 text-[13px] hover:bg-gray-100 dark:hover:bg-white/5 dark:text-gray-200"
                        onClick={() => {
                ...
                >
                  ...
                  {showProgramFilter && (
                    <div className="animate-dropdown animate-fadein absolute top-8 left-0 z-20 ml-2 min-w-[150px] rounded border border-gray-300 bg-white dark:border-white/10 dark:bg-[#1a1a1a] p-1 shadow-lg">
                      <div
                        className="flex cursor-pointer items-center gap-2 rounded-sm py-2 text-sm hover:bg-gray-100 dark:hover:bg-white/5 dark:text-gray-300"
                        onClick={() => {
      ...
      {showAddModal && (
        <>
          <div className="subject-selector-theme open-sans bg-opacity-40 lightbox-bg fixed inset-0 z-100 flex items-end justify-center min-[448px]:items-center">
