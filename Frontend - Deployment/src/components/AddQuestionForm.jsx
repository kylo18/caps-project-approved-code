...
const CombinedQuestionForm = ({
  ...
  useEffect(() => {
    ...
    const fetchExamQuestionsEnabled = async () => {
      ...
      try {
        // Parent components own this state via propIsExamQuestionsEnabled.
        // Keep this lifecycle only so dependent UI state doesn't regress.
      } catch (error) {
        // no-op
      } finally {
  ...
  const handleSubmit = async () => {
    ...
    if (formData.purpose_id === 1 && propIsExamQuestionsEnabled === false) {
      setError(
