      ...
      if (existingChoice) {
        ...
        result.push({
          personalQuizChoiceID:
            existingChoice.personalQuizChoiceID || existingChoice.id || null,
          choiceText: hasImage ? "" : displayChoiceText, // No text when choice has image
          isCorrect: isCorrect,
          image:
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          `Server returned invalid response. Status: ${questionResponse.status}`,
        );
      }

      if (!questionResponse.ok) {
        if (questionDataResult.errors) {
          const errorMessages = Object.values(questionDataResult.errors)
            .flat()
            .join(", ");
          throw new Error(
            errorMessages || questionDataResult.message || "Validation failed.",
          );
        }
        throw new Error(
      ...
      if (!questionDataResult.success) {
        ...
        throw new Error(
          questionDataResult.message || "Failed to update question.",
        );
      }
      ...
      choicesToSend.forEach((choice, index) => {
        if (choice.personalQuizChoiceID) {
          choicesFormData.append(
            `choices[${index}][personalQuizChoiceID]`,
      ...
      try {
        ...
        throw new Error(
          `Server returned invalid response for choices. Status: ${choicesResponse.status}`,
        );
      }

      if (!choicesResponse.ok) {
        if (choicesDataResult.errors) {
          const errorMessages = Object.values(choicesDataResult.errors)
            .flat()
            .join(", ");
          throw new Error(
            errorMessages ||
              choicesDataResult.message ||
              "Validation failed for choices.",
          );
        }
        throw new Error(
          choicesDataResult.message ||
      ...
      if (!choicesDataResult.success) {
        ...
        throw new Error(
          choicesDataResult.message || "Failed to update choices.",
        );
      }
      ...
      const updatedQuestion = {
        ...
        choices:
          choicesDataResult.choices || questionDataResult.quizQuestion.choices,
        personalQuizChoices:
          choicesDataResult.choices ||
          questionDataResult.quizQuestion.personalQuizChoices,
      };
